...
def test_catboost_training(config):
    ...
    trainer.train(iter(train_data), test_df)
    ...
    test_pool = Pool(
        X_test, y_test,
        cat_features=trainer.model_config.get('cat_features', []),
        text_features=trainer.model_config.get('text_features', []),
        embedding_features=trainer.model_config.get('embedding_features', []),
    )

    acc = trainer.evaluate(test_pool)
    assert 0 <= acc <= 1
